...
public class CouponCodeValidator {
    public List<String> validateCoupons(String[] code, String[] businessLine, boolean[] isActive) {
        ...
        List<String> categories = List.of("electronics", "grocery", "pharmacy", "restaurant");
        for (int i = 0; i < code.length; i++) {
            String c = code[i];
            String b = businessLine[i];
            if (p.test(c) && isActive[i] && categories.contains(b)) {
                map.compute(b, (key, old) -> {
        ...
        for (String category : categories) {
            List<String> codes = map.getOrDefault(category, new ArrayList<>());
            codes.sort(Comparator.comparing(x -> x));
            ans.addAll(codes);
        }
        return ans;
    }
